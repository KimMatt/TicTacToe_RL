...
def load_agent(filename):
    algo_type = filename.split("_")[0]
    assert algo_type in algo_types, print("The selected model file is not properly named [algo_type]_[params...].p is expected")
    agent = Agent(algo_type)
            ...
            if event.type == MOUSEBUTTONUP:
                ...
                if box is not None and game_model.in_progress and box in game_model.possible_moves:
                    ...
                    if game_model.in_progress:
                        ...
                        pygame.display.flip()
                        time.sleep(0.5)
                        action = agent.get_action(game_model.state)
                        game_model.play_move(0,action)
